...
pub fn t(locale: Locale, msg: Msg) -> &'static str {
    match msg {
        Msg::SignInRequired => match locale {
            Locale::En => "Sign-in required. Send an `Authorization: Basic …` header or open the app and sign in again.",
            Locale::ZhHans => "需要登录。请携带 `Authorization: Basic …` 请求头，或在应用中重新登录。",
        },
        Msg::BasicScheme => match locale {
            Locale::En => "Authorization must use Basic authentication (`Authorization: Basic <base64>`). Check the header format.",
            Locale::ZhHans => "Authorization 须使用 Basic 认证（`Authorization: Basic <base64>`）。请检查请求头格式。",
        },
        Msg::CredentialsDecode => match locale {
            Locale::En => "Credentials could not be decoded. Re-enter your username and password and try again.",
            Locale::ZhHans => "无法解析凭据。请重新输入用户名和密码后重试。",
        },
        Msg::CredentialsMismatch => match locale {
            Locale::En => "Username or password did not match the server configuration. Check your credentials and try again.",
            Locale::ZhHans => "用户名或密码与服务器配置不一致。请核对凭据后重试。",
        },

        Msg::InvalidFolderPath => match locale {
            Locale::En => "The path must be a workspace folder path: use `/` for the root, or `/notes/` or `/team/docs/` for subfolders (always start with `/`, separate segments with `/`, end non-root paths with `/`). Do not use `..` or try to escape the workspace. Fix the path and try again.",
            Locale::ZhHans => "路径须为工作区内的文件夹路径：根目录用 `/`；子文件夹用 `/笔记/`、`/项目/文档/` 等形式（必须以 `/` 开头，各段用 `/` 分隔，非根路径须以 `/` 结尾）。不要使用 `..` 或试图跳出工作区。请修正路径后重试。",
        },
        ...
        Msg::TagNoControl => match locale {
            Locale::En => "Tags cannot contain control characters. Use letters, numbers, and underscores.",
        ...
        Msg::MultipartReadFailed => match locale {
            Locale::En => "Could not read the multipart form. Retry the upload and ensure the request is multipart/form-data.",
            Locale::ZhHans => "无法读取 multipart 表单。请重试上传，并确保请求为 multipart/form-data。",
        },
        ...
        Msg::MultipartMultipleDocId => match locale {
            Locale::En => "The form contains more than one `doc_id` field. Send a single doc_id value.",
            Locale::ZhHans => "表单中包含多个 `doc_id` 字段。请只发送一个 doc_id 值。",
        },

        Msg::DocIdMustBeNumber => match locale {
            Locale::En => "`doc_id` must be a non-negative whole number (or omit / use 0 for a new document).",
            Locale::ZhHans => "`doc_id` 必须是非负整数（新建文档可省略或使用 0）。",
        },
        Msg::NewDocNeedsFilePart => match locale {
            Locale::En => "New documents need a `file` part in the multipart body. Add the file field and try again.",
            Locale::ZhHans => "新建文档需要在 multipart 正文中提供 `file` 部分。请添加文件字段后重试。",
        },
        Msg::UploadedFileUtf8 => match locale {
            Locale::En => "The uploaded file must be valid UTF-8 text or HTML. Convert the encoding and try again.",
            Locale::ZhHans => "上传的文件须为有效的 UTF-8 文本或 HTML。请转换编码后重试。",
        },

        Msg::BulkBatchSize => match locale {
            Locale::En => "Bulk index expects between 1 and 200 documents in one request. Reduce or split the batch and try again.",
        ...
        Msg::NewDocNeedsContent => match locale {
            Locale::En => "New documents require body content. Paste or type content in the editor, then save.",
            Locale::ZhHans => "新建文档需要正文。请在编辑器中粘贴或输入内容后保存。",
        },

        Msg::SearchTooManyTags => match locale {
            Locale::En => "Too many tag filters in this search. Remove some tag filters (comma-separated) and try again.",
            Locale::ZhHans => "本次搜索的标签过滤过多。请减少部分标签（逗号分隔）后重试。",
        },
        Msg::SearchNeedsCriteria => match locale {
            Locale::En => "Enter search text and/or pick a folder path or tag filter. At least one of these is required.",
            Locale::ZhHans => "请输入搜索词，和/或选择文件夹路径或标签过滤。至少需要其中一项。",
        },

        Msg::DocIdPositive => match locale {
            Locale::En => "Document ID must be a positive number. Use the ID shown in search or the catalog.",
        ...
        Msg::NotFoundResource => match locale {
            Locale::En => "The requested resource was not found. Check the document ID or path and try again.",
            Locale::ZhHans => "未找到请求的资源。请核对文档 ID 或路径后重试。",
        },
        Msg::InternalServer => match locale {
            Locale::En => "Something went wrong on the server. Please try again in a moment; if the problem continues, contact support.",
...
pub fn search_query_parse_detail(locale: Locale, detail: &str) -> String {
    ...
    match locale {
        Locale::En => format!(
            "Search query could not be parsed ({d}). Adjust the syntax and try again."
        ),
